  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        if (!imagePath) {
          console.warn(`⚠️  No image mapping found for product: ${product.slug}`)
          errorCount++
          continue
        }
        ...
        if (imageError) {
          console.error(`❌ Failed to update image for ${product.name}: ${imageError.message}`)
          errorCount++
          continue
        }
    ...
    if (updatedCount > 0) {
      ...
      console.log('\n🔗 Test image URLs:')
      console.log(`Original: ${supabaseUrl}/storage/v1/object/public/product-images/shirts/classic-white-tshirt.jpg`)
      console.log(`Thumbnail: ${supabaseUrl}/storage/v1/render/image/public/product-images/shirts/classic-white-tshirt.jpg?width=200&height=200&quality=80&format=webp&resize=cover`)
    }

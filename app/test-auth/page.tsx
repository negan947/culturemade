...
export default function TestAuth() {
  ...
  const checkAuthState = useCallback(async () => {
    updateTest('Auth State Check', 'running', 'Checking current authentication state...');
    ...
    try {
      ...
      updateTest('Auth State Check', 'success', 
        user ? `Authenticated as ${user.email} (${profile?.role || 'no role'})` : 'Not authenticated',
        { user, profile }
      );
                } catch (error) {
         updateTest('Auth State Check', 'error', `Failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
       }
     }, [supabase]);
  ...
  const testRegistration = async () => {
    ...
    try {
      ...
      updateTest('User Registration', 'success', 
        `Registration successful for ${testEmail}`,
        { user: data.user, session: data.session }
      );
         } catch (error) {
       updateTest('User Registration', 'error', `Registration failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
  };
  ...
  const testLogin = async () => {
    ...
    try {
      ...
      await checkAuthState();
       ...
       updateTest('User Login', 'error', `Login failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
  };
  ...
  const testLogout = async () => {
    ...
    try {
      ...
      await checkAuthState();
       ...
       updateTest('User Logout', 'error', `Logout failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
   };

   // Test protected route access
   const testProtectedRoutes = async () => {
     updateTest('Protected Routes', 'running', 'Testing protected route access...');
     ...
     try {
       ...
       updateTest('Protected Routes', 'success', 
         'Protected routes tested',
         results
       );
     } catch (error) {
       updateTest('Protected Routes', 'error', `Protected routes test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
  };
  ...
  const testAdminRoutes = async () => {
    ...
    try {
      ...
      updateTest('Admin Routes', 'success', 
        `Admin route tested (Status: ${adminResponse.status})`,
        results
      );
         } catch (error) {
       updateTest('Admin Routes', 'error', `Admin routes test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
   };

   // Test rate limiting
   const testRateLimiting = async () => {
     updateTest('Rate Limiting', 'running', 'Testing rate limiting on auth routes...');
     ...
     try {
       ...
       updateTest('Rate Limiting', 'success', 
         `Rate limiting test completed. ${auths.length} attempts processed, ${errors.length} blocked`,
         { results: results.map(r => r.status) }
       );
     } catch (error) {
       updateTest('Rate Limiting', 'error', `Rate limiting test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
  };

  // Test password reset
  const testPasswordReset = async () => {
    updateTest('Password Reset', 'running', 'Testing password reset functionality...');
      ...
      updateTest('Password Reset', 'success', 
        `Password reset email sent to ${testCredentials.email}`
      );
         } catch (error) {
       updateTest('Password Reset', 'error', `Password reset failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
   };
   ...
   const testProfileCreation = async () => {
     ...
     try {
       ...
       if (existingProfile) {
         updateTest('Profile Creation', 'success', 
         ...
         const { data: newProfile, error: createError } = await supabase
           ...
           .insert({
             ...
             role: 'customer',
             full_name: (authState.user.user_metadata?.full_name as string) || 'Test User'
           })
           .select()
           .single();
       ...
       updateTest('Profile Creation', 'error', `Profile creation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
     }
  };
  ...
  return (
    <div className="container mx-auto p-8 max-w-6xl">
      <h1 className="text-4xl font-bold mb-6">🔐 Authentication System Test Suite</h1>
      
      {/* Auth State Summary */}
      <div className="mb-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
        <h2 className="text-2xl font-semibold text-blue-800 mb-4">Current Authentication State</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-3 bg-white rounded border">
            <span className="text-sm text-gray-600">Status:</span>
            <p className="font-semibold">{authState.isAuthenticated ? '🟢 Authenticated' : '🔴 Not Authenticated'}</p>
          </div>
          <div className="p-3 bg-white rounded border">
          ...
          <div className="p-3 bg-white rounded border">
            ...
            <p className="font-semibold">{authState.isAdmin ? '🔑 Yes' : '👤 No'}</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          <div>
            <strong>Supabase URL:</strong> {process.env.NEXT_PUBLIC_SUPABASE_URL ? '✅ Configured' : '❌ Missing'}
                     ...
                     <div>
             <strong>Auth Callback:</strong> {typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000'}/api/auth/callback
           </div>
        </div>
      </div>
    </div>
